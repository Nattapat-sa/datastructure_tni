	...
	public void delete(int target) {
		...
		if(root == null) {
			...
			if(deleteNode.left == null && deleteNode.right == null) {
				if(parents.left != null && parents.left.data == target) {
					parents.left = null;
				}
				if(parents.right != null && parents.right.data == target) {
					parents.right = null;
				}
			...
			//Case 3: Delete a node with 1 child
				...
				if(deleteNode.left != null) {
					if(parents.left.data == deleteNode.data) {
						parents.left = deleteNode.left;
					} else {
						parents.right=deleteNode.left;
					}
				} else {
					if(parents.left.data == deleteNode.data) {
						parents.left = deleteNode.right;
					} else {
						parents.right = deleteNode.right;
					}
				}
			}
		}
